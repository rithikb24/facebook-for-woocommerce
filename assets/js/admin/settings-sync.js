...
jQuery( document ).ready( function( $ ) {
	...
	// handle the sync button click (modified products)
	...
	// handle the sync all products button click
	$( '#woocommerce-facebook-settings-sync-modified-products' ).click( function( event ) {
		event.preventDefault();
		if ( confirm( facebook_for_woocommerce_settings_sync.i18n.confirm_sync_all ) ) {
			setProductSyncStatus();
			let startTime = Date.now();
			$.post( facebook_for_woocommerce_settings_sync.ajax_url, {
				action: 'wc_facebook_sync_modified_products',
				nonce:  facebook_for_woocommerce_settings_sync.sync_all_products_nonce,
			}, function ( response ) {
				if ( ! response.success ) {
					let error = facebook_for_woocommerce_settings_sync.i18n.general_error;
					if ( response.data && response.data.length > 0 ) {
